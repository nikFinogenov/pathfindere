...
static void print_invalid_number(t_islands *path) {
    mx_printerr("error: invalid number of islands\n");
    mx_clean_memory_path(path);
    exit(1);
}
static void unique_islands(t_islands *path) {
    int num = 1;
...
static void double_islands(t_islands *path) {
    int line = 2;
...
static void duplicate_bridges(t_islands *path) {
